...
* [FileGetCurrentFile consolidation](docs/filegetcurrentfile-closure.md) — A1A8 developer-facing SDK contract: no arguments, read-only active-descriptor query, canonical 16-bit token return, direct A-line exposure, historical ABI evolution and regression status.
...
* [SYS_A248 consolidation](docs/sys-a248-closure.md) — A248 developer-facing SDK contract: read-only global file-password-protection state getter, byte-only return contract, neutral-name policy, direct A-line exposure and regression guidance.
...
* [Transport payload-limit closure](docs/transport-payload-limit-closure.md) — closes A288 mechanically as a no-argument 16-bit getter for the current payload-byte limit admitted by the correlated internal send path. The neutral prototype is in `os3k/transport_payload.h`.
* [IrDA state-initializer closure](docs/irda-state-init-closure.md) — closes A28C mechanically as a four-slot singleton-state initializer with mandatory names/data callback, optional event callback, byte return and a cross-ROM secondary-string bounds defect. The neutral prototype is in `os3k/irda_state_init.h`.
