...
Two independent public implementations agree that selector `0x06` is only one stage of a multi-request installation transaction, not the request that carries the applet payload itself.
...
Neotools resolves the host-side ordering more clearly. Its applet installer performs these stages:
...
- applet bytes are transferred in blocks of at most `0x400` by the observed host implementation;
- `0x02` announces each block and its checksum, with responses `0x42` and `0x43` around the raw block transfer;
- `0x0b` is issued after every block and receives `0x47`;
