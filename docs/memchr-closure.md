...
in AS3000 2005, NEO 2005 and NEO 2013. Its SHA-256 is
...
one. Each constructs exactly three longword argument slots. The Keywords Wireless
site performs an immediate 0x0C-byte cleanup and `TST.L D0`; the other sites consume
D0.L before their later stack/frame cleanup rather than providing an equivalent
immediate-cleanup observation. Counts include 0x400 and 0x800, while the Keywords
Wireless site supplies a dynamically computed full-long count. The caller evidence
therefore independently confirms the three-slot ABI and pointer/NULL D0.L result.
...
match-pointer correction, NULL path, official callers, and cross-ROM identity. It
executed with `OVERALL PASS` for all three ROMs. Dynamic/emulator validation has not
been executed and is not claimed here.
