...
- Structural distinction: the syscall stub for index 264 emits the Motorola 68k Line-A opcode `A420`. This opcode must not be conflated with absolute runtime address `0x00400420`.
...
- Direct firmware xrefs: zero JSR/JMP/BSR.W/BSR.B to the A420 syscall in the canonical NEO 2013 main ROM. Official SmartApplet exposure is structurally absent (`0/41`) for this late slot.
- Caller retraction: Manager selector `0x18` is **not** an A420 caller. Its literal `0x00400420` is a physical executable entry in the independently stored Small ROM; that entry begins with privileged status-register setup and jumps to another Small-ROM routine. Its first opcode is not `A420`. The prior `Manager18 -> A420 -> A3DC` interpretation is withdrawn.
- Consequence: Manager selector `0x18` supplies no evidence for A420 argument semantics, A3DC slot 1, or a Small-ROM transition helper inside A3DC. A420 remains a valid standalone ABI wrapper solely on its independent table/handler/caller evidence.
...
- Static regression: historical `23/23 PASS` plus an independent `36/36 PASS`; fresh source-first primary revalidation `32/32 PASS / OVERALL PASS`. The separate cross-ROM category-correction regression is `26/26 PASS`.
